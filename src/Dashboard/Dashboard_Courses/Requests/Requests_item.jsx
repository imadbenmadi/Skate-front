...
function Requests_item({ request, onDelete }) {
    ...
    async function handle_accept_request(UserId, CourseId) {
        try {
            setAccept_Loading(true);
            const response = await axios.post(
                "https://backend.skate-consult.com/Dashboard/Courses/Requests/Accept",
                { UserId, CourseId },
                {
    ...
    async function handle_reject_request(UserId, CourseId) {
        try {
            setReject_Loading(true);
            const response = await axios.post(
                "https://backend.skate-consult.com/Dashboard/Courses/Requests/Reject",
                { UserId, CourseId },
                {
                    withCredentials: true,
                    validateStatus: () => true,
    ...
    return (
        ...
        >
            ...
            <td
                ...
                className="w-[150px] whitespace-nowrap border overflow-auto scrollbar-thumb-rounded-full scrollbar-thin scrollbar-thumb-green scrollbar-track-slate-300"
            >
                {request.Course.Title ? request.Course.Title : "none"}
            </td>
            <td
                style={{ maxWidth: "70px" }}
            ...
            >
                {request.Course.Price ? request.Course.Price + " DA" : "none"}
            </td>
            <td
                style={{ maxWidth: "150px" }}
